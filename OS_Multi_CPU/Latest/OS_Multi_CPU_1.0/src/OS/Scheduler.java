...
public class Scheduler extends threadAsymmetric{
    ...
    public void setMDisktoRAM() {
        ...
        synchronized (oSystem.getRam()){
            while (oSystem.getJobQueue().lookInto().getProcessSize() < oSystem.getRam().getSize()
                    && oSystem.getJobQueue().size() >0){
    ...
    /*
    Controls each Thread due to scheduling
    ...
    //public  void turnOn(){
        ...
        while (!Thread.currentThread().isInterrupted()){
            while (oSystem.sleepAllCPUs()==false){
                try {
                    Thread.sleep(5);        // or threadAsymmetric ???
                } catch (InterruptedException e) {
                    System.out.println("error with sleeping CPUs in turnOn-Scheduler");
                }
            }
            if (oSystem.endCPUs()) { //finished given in process in CPU
                setMRamtoDisk();

                for (int i = 0; i < 30; i++) {
                    ControlBlock controlBlock = oSystem.getTerminatedQueue().getIndex(i);
                    System.out.println("Job in Terminated Queue" + controlBlock.getProgramID());
                }
                long currentT = System.currentTimeMillis();
                long endT = currentT - oSystem.getSimStartTime();
                System.out.println("Time it Took: " + endT);

                Thread.currentThread().interrupt();

                end();
                OSystem.getInstance().stopT(); // ??? actual thread call not working???
            }
            else {
                mSchedule();
    ...
    /*
       ...
       try {
           ...
           if ((!controlBlock.isInMemory())  && (controlBlock.getStatus() != ControlBlock.Status.TERMINATED)
               ...
               //loadProcessIntoMemory(pcb.getProgramID());// load each process into RAM
...
/*
 ...
 /*   public void FIFOLongTermScheduler() {
        ...
        //for (int i = 0; i < disk.searchPCB(i).getProgramID(); i++) {
            System.out.println("disk search pcb program counter: " + disk.searchPCB(count).getProgramID());
        ...
        System.out.println(disk.searchPCB(i).getProgramID());
       while (disk.searchPCB(i).getProgramID()<=30){
    ...
    /*   public void PriorityLongTermScheduler() {
           ...
           for (ControlBlock pcb : temporaryPCB) {
               if (!pcb.isInMemory() && ram.hasSpaceForProcess(pcb) && pcb.getStatus() != FINISHED) {//**********************************************************************8
                   ...
                   loadProcessIntoMemory(pcb.getProgramID());
               }
           }
       }
